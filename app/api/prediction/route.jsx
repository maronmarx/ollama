...
export const POST = async (req) => {
  if (req.method === 'POST') {
    const { tableName, nomProject, projectDirectory, nbrModels } = await req.json();

    console.log("tableName: " + tableName);
    console.log("nomProject: " + nomProject);
    console.log("projectDirectory: " + projectDirectory);
    console.log("nbrModels: " + nbrModels);

    // Construction de la commande en fonction de la présence de nbrModels
    const command = nbrModels ?
      `Rscript /app/prediction.R ${tableName} ${nomProject} ${projectDirectory} ${nbrModels}` :
      `Rscript /app/prediction.R ${tableName} ${nomProject} ${projectDirectory}`;
